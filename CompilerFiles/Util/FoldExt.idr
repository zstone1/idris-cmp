...
|||Given an associative folding function, this proves that 
|||we can pull of the first element in the obvious way.
FoldAssocCons: Associates f ->
               (a:t) -> 
               (as:Vect n t) -> 
               a `f` (foldr f s as) = foldr f s (a :: as)  
...
FoldAssocCons {f} {s} prf a (x :: xs) =
  ...
  (a `f` (foldr f s (x::xs)))    ={ rewrite rec in axsAssoc  }=
  ((a `f` x) `f` (foldr f s xs)) ={ FoldAssocCons {f}{s} prf (a `f` x) xs }=
  (foldr f s ((a `f` x) :: xs))  ={ let extends = (AssociatesExtend prf a x) in 
                                       cong {f=\e => foldrImpl f s e xs} extends }=
...
FoldAssocConcat : Associates f ->
                  ...
                  (as : Vect n t) ->
                  (bs : Vect m t) ->
                  (foldr f s as) `f` (foldr f s bs) = foldr f s (as ++ bs) 
FoldAssocConcat {f} {s} prf idprf [] bs = rewrite idprf (foldr f s bs) in Refl
FoldAssocConcat {f} {s} prf idprf (a :: as) bs = 
  ...
  ((foldr f s (a::as)) `f` (foldr f s bs))    ={ rewrite consAAs in assocAAsBs }=
  (a `f` ((foldr f s as) `f` (foldr f s bs))) ={ cong $ FoldAssocConcat {f}{s} prf idprf as bs }=
  (a `f` (foldr f s (as ++ bs)))              ={ FoldAssocCons {f}{s} prf a (as++bs) }=
  (foldr f s (a :: (as++bs)))                 ={ Refl }= 
